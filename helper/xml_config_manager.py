...
class ConfigManager:
    ...
    def get_with_default(self, *path, default=None):
        self.logger.debug(f"get_with_default called with path={path}, default={default}")
        try:
            result = self.get(*path)
            self.logger.debug(f"Found value: {result}")
            return result
        except KeyError:
            self.logger.debug(f"KeyError for path={path}, _defaults={self._defaults}")
            if path and path[0] in self._defaults:
                current = self._defaults[path[0]]
                self.logger.debug(f"_defaults[{path[0]}]: {current}")
                for key in path[1:]:
                    if isinstance(current, dict) and key in current:
                        current = current[key]
                        self.logger.debug(f"Current after {key}: {current}")
                    else:
                        self.logger.debug(f"Key '{key}' not found in _defaults, returning default: {default}")
                        return default
                self.logger.debug(f"Returning: {current if current is not None else default}")
                return current if current is not None else default
            self.logger.debug(f"No _defaults match, returning default: {default}")
            return default
